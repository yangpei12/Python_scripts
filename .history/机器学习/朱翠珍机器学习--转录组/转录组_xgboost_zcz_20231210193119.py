...
gs = GridSearchCV(estimator=pipe_xgb, param_grid=param_grid, cv=stratified_kfold, n_jobs=-1, refit=True)
...
final_result.write(xgboost_cross_val_score)

# 使用测试集估计模型的性能
clf = gs.fit(X=X_train, y=Y_train)
xgboost_Test_accuracy = 'Test accuracy: {0:.3f}\n'.format(clf.score(X_test, Y_test))
final_result.write(xgboost_Test_accuracy)

# 模型评估--学习曲线
import matplotlib.pyplot as plt
from sklearn.model_selection import learning_curve
train_sizes, train_scores, test_scores = learning_curve(estimator = pipe_xgb, X=X_train, y=Y_train, 
...
plt.savefig('xgboost_learning_curve.pdf')
plt.show()
...
xgboost_Precision = 'Precision: {0:.3f}\n'.format(pre_val)
final_result.write(xgboost_Precision)

rec_val = recall_score(y_true= Y_test, y_pred=Y_pred, average='macro')
xgboost_Recall = 'Recall: {0:.3f}\n'.format(rec_val)
final_result.write(xgboost_Recall)
final_result.close()
...
plt.legend(bbox_to_anchor=(1, 0), loc=3, borderaxespad=0)
plt.savefig('xgboost_ROC_curve.pdf')
plt.show()