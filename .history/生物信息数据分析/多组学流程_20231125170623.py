...
# 相关性结果的输出
output_name = '{0}/Integrative_Analysis/{0}_mRNA_meta_corr_result.txt'.format(cond, cond)
mRNA_meta_corr_result.to_csv(output_name, sep='\t', index=False)

# 差异表达矩阵

diff_mRNA_exp_matrix.to_csv('{0}/Integrative_Analysis/{0}_mRNA_exp_matrix.txt'.format(cond, cond), sep='\t', index=False)
diff_meta_exp_matrix.to_csv('{0}/Integrative_Analysis/{0}_meta_exp_matrix.txt'.format(cond, cond), sep='\t', index=False)
