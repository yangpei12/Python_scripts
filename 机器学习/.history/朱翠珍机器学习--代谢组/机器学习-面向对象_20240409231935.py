...
class ml_algorithm:
    ...
    def sha(self, best_model, algorithm):
        ...
        shap.force_plot(explainer.expected_value, shap_values[0,:], 
                        X[0, :], show=False, matplotlib=True).savefig('%s_force.png'%algorithm)
...
if __name__ == "__main__":
    init_model = ml_algorithm(X_train, X_test, y_train, y_test)
    svm_model = init_model.svm()
    confusion_matrix = init_model.confusion_matrix_est(svm_model)
    force_plot = init_model.sha(svm_model, 'svm')